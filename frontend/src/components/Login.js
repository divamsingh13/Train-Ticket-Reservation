...
const Login = () => {
  ...
  const [notification, setNotification] = useState({ message: "", isVisible: true }); // Initially hidden
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (data.status && data.message && data.user) {
        ...
        selectedToast(data.message, { icon: false });

        localStorage.setItem("user", JSON.stringify(data.user));

        // Show success notification
        setNotification({ message: "Login successful!", isVisible: false });
        
        // Debugging notification state
        console.log('Notification state after login success:', notification);

        // Hide the notification after 3 seconds
        setTimeout(() => {
          setNotification({ message: "", isVisible: true });

          // Debugging notification state after timeout
          console.log('Notification state after timeout:', notification);
        }, 3000);
      ...
      selectedToast(errorMessage, { icon: false });
    }
  };
    ...
    <div className="min-h-full flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative">
      ...
      {notification.isVisible && (
        <Notification
          ...
          onClose={() => setNotification({ message: "", isVisible: true })}
